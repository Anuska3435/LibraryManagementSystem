...
const EditBook = () => {
  ...
  // Fetch categories & existing books on mount
  useEffect(() => {
    fetchExistingData();
  }, []);

  // Fetch book data by ID if editing
  useEffect(() => {
    if (id) {
      fetchBook();
    }
  ...
  const fetchBook = async () => {
    try {
      ...
      if (res.data) {
        // Convert publishedYear to string for input value
        setBook({
          ...res.data,
          publishedYear: res.data.publishedYear?.toString() || "",
        });
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        ...
        // Add new book (optional fallback)
            ...
            {success && (
              <div className="flex items-center p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <span className="text-green-800 font-medium">
                  Book {id ? "updated" : "added"} successfully!
                </span>
              </div>
            )}
